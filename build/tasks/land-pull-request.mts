 ...
 *
 * Run as `pnpm land <number>`, or `pnpm land <number> --dry-run` to see the
...
import { validateCommitMessage } from '../shared/commit-message.mts';
import {
  checksVerdict,
  composeLandingMessage,
  partsOfMessage,
} from '../shared/landing.mts';
...
try {
  if (number === undefined || !/^\d+$/.test(number)) {
    console.error('Usage: pnpm land <number> [--dry-run]');
    process.exitCode = 1;
  } else {
