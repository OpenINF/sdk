...
describe('yellow', () => {
  ...
  it('should colorize the string yellow when ANSI is supported', () => {
    mock.module('../../src/_internal/supports-ansi.js', {
      exports: { supportsAnsi: () => true },
    });
    const { yellow } = freshRequire('../../src/helpers/yellow');
    const result: string = yellow('foo');
  ...
  it('should return the string unchanged when ANSI is unsupported', () => {
    ...
    const { yellow } = freshRequire('../../src/helpers/yellow');
    assert.strictEqual(yellow('foo'), 'foo');
  });
});
