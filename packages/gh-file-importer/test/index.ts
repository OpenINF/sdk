...
describe('GhFileImporter', () => {
  ...
  describe('importUrl', () => {
    ...
    it('should create a destination directory that does not exist yet', async () => {
      // The physical root has to be resolved to be compared against, and
      // resolving one that is not there is an `ENOENT`. Importing into a
      // fresh directory is the ordinary case, not an edge one.
      mockOctokit(() => {
        throw new Error('should not be called');
      });
      mockFetch('from url');
      ...
      const base = await mkdtemp(pathJoin(tmpdir(), 'gh-file-importer-'));
      try {
        const destDir = pathJoin(base, 'not', 'created', 'yet');
        const importer = new GhFileImporter({ destDir });
        const written = await importer.importUrl('https://example.com/a.txt');

        assert.strictEqual(written, pathJoin(destDir, 'a.txt'));
    ...
    it('should refuse a URL whose pathname names a directory', async () => {
      // `basename('/releases/')` answers `releases` rather than nothing, so
      // the empty-filename check accepts it and the response is written as a
      ...
      try {
        ...
        for (const url of [
          ...
          await assert.rejects(
            ...
            /pathname names a directory|does not contain a filename/
          );
        }

        // An explicit destination is the caller's to choose, so the shape of
        // the URL stops mattering.
        assert.strictEqual(
          await importer.importUrl(
            'https://example.com/releases/',
            'chosen.txt'
          ),
          pathJoin(destDir, 'chosen.txt')
        );
      } finally {
        await rm(destDir, { recursive: true, force: true });
      }
...
describe('path traversal', () => {
  ...
  it('should create no directory outside destDir on the way to refusing', async () => {
    // `mkdir` with `recursive` follows a symlink it meets, so a path checked
    // only after the directories exist has already made some of them in the
    // wrong place. Refusing the write is not enough on its own.
    mockOctokit(() => {
      throw new Error('should not be called');
    });
    mockFetch('payload');
  ...
  it('should refuse a final-component symlink', async () => {
    ...
    try {
      ...
      // Named rather than left as whatever errno the platform raises: on
      // POSIX the open fails with `ELOOP`, which says nothing about the rule.
      await assert.rejects(
