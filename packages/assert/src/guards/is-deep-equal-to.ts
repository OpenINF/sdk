...
function deepEqual(
  ...
  // `length` is not enumerable, and a hole is not a key, so two arrays of
  // different lengths can present the same key set: `new Array(1)` and
  // `new Array(2)` both enumerate nothing. Length is the only thing that
  // tells them apart.
  if (aIsArray && a.length !== (b as unknown[]).length) {
    return false;
  }
