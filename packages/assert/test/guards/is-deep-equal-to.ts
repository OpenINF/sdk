...
describe(isDeepEqualTo.name, () => {
  ...
  it('should compare array lengths, which no key set reveals', () => {
    // Holes are not keys and `length` is not enumerable, so these pairs have
    // identical enumerable key sets and different contents.
    assert.strictEqual(isDeepEqualTo(new Array(1))(new Array(2)), false);
    assert.strictEqual(
      isDeepEqualTo([1, 2, 3])(Object.assign([1, 2, 3], { length: 4 })),
      false
    );
    assert.strictEqual(isDeepEqualTo([1, 2])([1, 2]), true);
  });
