...
# .nvmrc is the only place this repository names a Node version, so there is
# nothing to hold it against. nvm.fish reads the file itself; the image's nvm
# is handed its contents below.
...
Ready, on Node $(node -v).
  ...
  pnpm test    the package test suites
  pnpm lint    oxlint, per package

CI also runs lint:format, lint:knip, lint:spelling and lint:packages.
